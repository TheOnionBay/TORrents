...
class Node(Flask):
    ...
    def main_handler(self):
        """Handles multiple types of received messages:
        1) bridge -> downstream
        2) downstream -> upstream or downstream -> bridge
    ...
    def create_tunnel(self, message, colour):
        ...
        if "relay" not in payload or "to" not in payload:
            # All these fields should be present else -> bad request
            return "'relay' and 'to' fields are needed in payload when creating the tunnel", 400
    ...
    def make_bridge(self, fsid, bridge_cid, bridge_ip, colour):
        """Adds a new entry to the Upstream File Sharing Table.
        """
    ...
    def receive_bridge(self, bridge_cid, origin_cid, colour):
        """Adds a new entry to the Downstream File Sharing Table.
        """
    ...
    def cprint(self, args, id, colour):
        """Displays log statements generated by the node.
        """
